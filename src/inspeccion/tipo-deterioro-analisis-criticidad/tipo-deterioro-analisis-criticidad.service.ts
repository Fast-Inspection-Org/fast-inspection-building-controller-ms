...
export class TipoDeterioroAnalisisCriticidadService {
    ...
    public async getTiposDeteriorosAnalisisCriticidadConfigurados(materialId: string /* representa el identificador del material la cual pertenecen los tipos de deterioro */,
        ...
        for (let index = 0; index < createTiposDeteriorosAnalisisCriticidadDTO.length; index++) {
            ...
            const deteriorosTipoDeterioro: Array<Deterioro> = function (): Array<Deterioro> {
                ...
                deteriorosInspeccion.forEach((deterioroInspeccion) => {
                    if (deterioroInspeccion.tipoDeterioroId === createTipoDeterioroAnalisisCriticidadDTO.id) // si es un deterioro de dicho tipo de deterioro
                        deteriorosSchemas.push(new Deterioro(deterioroInspeccion.codigo, deterioroInspeccion.campos))
                })

                return deteriorosSchemas
            }()
            tiposDeteriorosAnalisisCriticidad.push(new TipoDeterioroAnalisisCriticidad(createTipoDeterioroAnalisisCriticidadDTO.id,
                createTipoDeterioroAnalisisCriticidadDTO.nombre, deteriorosTipoDeterioro, sistema, inspeccion, this,
                createTipoDeterioroAnalisisCriticidadDTO.detectabilidad, createTipoDeterioroAnalisisCriticidadDTO.camposAfectados))
        }
    ...
    public async getIndiceProgramado(configVersion: string, calculo: Calculos, valor: number): Promise<CreateIndicadorDTO> {
        return await firstValueFrom(this.configsClient.send('getIndicadorCalculo', {
